<script>
  ...
  function parseExercises(description) {
    ...
    for (const fragment of fragments) {
      const fallbackMatch = /(.+?)[:]?\s*(\d+)\s*sets?\s*(?:x|×|of)\s*(\d+)\s*reps?/i.exec(fragment)
  ...
  async function regenerateSelectedDay() {
    ...
    tasks.set(await api.getTasks($selectedProfile.id));
    status = result.length > 0 ? `Regenerated ${result.length} tasks for ${dateString}.` : 'No tasks regenerated.';
  }
  ...
  async function downloadTasksPdf() {
    if ($tasks.length === 0) {
      status = 'No tasks available yet. Generate the calendar before exporting.';
      return;
    }
    ...
    try {
      ...
      for (const date of Object.keys(grouped).sort()) {
        ...
        grouped[date].forEach((task) => {
          ...
          const exercises = task.category === 'workout' ? parseExercises(task.description) : null;
  ...
  $: selectedIso = selectedDate ? selectedDate.toISOString().slice(0, 10) : null;
  $: todayTasks = $tasks.filter((task) => task.date.slice(0, 10) === todayIso);
  $: selectedDayTasks = selectedIso ? $tasks.filter((task) => task.date.slice(0, 10) === selectedIso) : [];
...
<AppShell title="Calendar">
  <div class="space-y-4">
    <section class="glass-panel rounded-[2rem] p-6">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-wrap gap-3">
          <button class="rounded-full bg-ember px-5 py-3 font-semibold text-black" on:click={regenerateMonth}>Refresh AI Tasks</button>
          <button class="rounded-full border border-white/10 bg-white/5 px-5 py-3 font-semibold text-white hover:bg-white/10" on:click={clearCalendar}>Clear Calendar</button>
          <button class="rounded-full border border-white/10 bg-white/5 px-5 py-3 font-semibold text-white hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed" on:click={downloadTasksPdf} disabled={$tasks.length === 0}>
            Download PDF
          </button>
        </div>
      </div>
    ...
    <div class="grid gap-4 xl:grid-cols-[1.6fr_0.9fr]">
      <div>
        <CalendarView tasks={$tasks} bind:viewDate selectedDate={selectedDate} on:select={handleSelect} />
      </div>
        ...
        <section class="glass-panel rounded-[2rem] p-6">
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <h3 class="mt-2 text-2xl font-semibold">{new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}</h3>
            </div>
            <span class="rounded-full bg-ember px-4 py-2 text-sm font-semibold text-black">{todayTasks.length} items</span>
          </div>
              ...
              {#each todayTasks as task}
                <div class="rounded-3xl border border-white/10 bg-black/20 p-4">
                  <div class="flex items-center justify-between gap-3">
                    <div>
                      <p class="text-sm text-stone-400">{task.category === 'workout' ? 'Workout' : 'Diet'}</p>
                      <h4 class="mt-1 font-semibold">{task.title}</h4>
                    </div>
                    <span class="rounded-full bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.15em] text-stone-300">{task.plan_scope ?? 'manual'}</span>
                  </div>
                      ...
                      <div class="mt-3 space-y-2">
                        {#each parseExercises(task.description) as exercise}
                          <div class="flex items-center gap-2 text-sm text-stone-300">
                            ...
                            <span><strong>{exercise.name}:</strong> {exercise.sets} sets × {exercise.reps} reps</span>
                          </div>
                        {/each}
                      </div>
                    {:else}
                      <p class="mt-3 text-sm leading-6 text-stone-300">{task.description}</p>
                    {/if}
                  {:else}
                    <p class="mt-3 text-sm leading-6 text-stone-300">{task.description}</p>
                  {/if}
          ...
          {:else}
            <p class="mt-5 text-sm text-stone-400">No tasks for today yet. Select a profile and regenerate tasks to populate your daily schedule.</p>
          {/if}
        </section>
    ...
    {#if selectedDayOpen}
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
        <section class="glass-panel relative w-[min(95%,900px)] rounded-[2rem] p-6" role="dialog" aria-modal="true" tabindex="-1">
          <div class="mb-5 flex items-center justify-between gap-4">
            <div>
              <p class="text-sm text-stone-400">Selected day</p>
              <h3 class="mt-2 text-2xl font-semibold">{selectedDate?.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</h3>
            </div>
            <div class="flex items-center gap-2">
              <button class="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm text-stone-100 hover:bg-white/10" on:click={closeSelectedDay}>
                ✕
              </button>
              <button class="rounded-full bg-ember px-4 py-2 text-sm font-semibold text-black" on:click={regenerateSelectedDay}>
                Regenerate day
              </button>
            </div>
          </div>
          ...
          {:else}
            <div class="rounded-3xl border border-white/10 bg-black/20 p-6 text-sm text-stone-400">
              <p>Click a day on the calendar to open its detail modal. Then remove or regenerate the selected day.</p>
            </div>
          {/if}
        </section>
      </div>
