<script>
  ...
  function resetForm() {
    form = {
      ...
      medicalConditions: [...defaultProfileValues.medicalConditions],
    };
  }
...
<section class="min-h-screen px-4 py-10 md:px-10">
  <div
    class="mx-auto max-w-6xl rounded-[2rem] border border-white/10 bg-[#0c0d10]/90 p-6 shadow-glow backdrop-blur md:p-10"
  >
    <div class="mb-10">
      <p class="text-sm uppercase tracking-[0.4em] text-amber-200/60">
        FitFusion
      </p>
      ...
      <p class="mt-3 max-w-2xl text-sm text-stone-400 md:text-base">
        Local-only fitness profiles with a Netflix-inspired selector, AI
        coaching, workout planning, and diet tracking.
      </p>
    </div>

    {#if error}
      <div
        class="mb-6 rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-200"
      >
        {error}
      </div>
    {/if}
      ...
      {:else}
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {#each profiles as profile}
            <div
              class="group rounded-[1.75rem] border border-white/8 bg-white/[0.02] p-5 text-left transition hover:-translate-y-1 hover:border-ember/40 hover:bg-white/[0.05]"
            >
              <div
                class="mb-5 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-[#ffd6bd] via-[#ffc299] to-[#ff8c00] text-3xl font-bold text-[#251200]"
              >
          ...
          >
            <div
              class="flex h-24 w-24 items-center justify-center rounded-full bg-[#ffd6bd] text-5xl text-[#1b130a]"
            >
              +
            </div>
            <span class="mt-5 text-lg font-semibold text-stone-200"
              >Create profile</span
            >
          </button>
        </div>
...
{#if showModal}
  <div class="fixed inset-0 z-40 overflow-y-auto bg-black/70 p-4">
    <div class="mx-auto w-full max-w-5xl glass-panel rounded-[2rem] p-6">
      <div class="mb-6 flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-stone-400">
            Profiles stay local in MongoDB. No login and no auth flow.
          </p>
        </div>
        <button
      ...
      <div class="space-y-8">
        <div>
          <h4 class="text-sm uppercase tracking-[0.3em] text-amber-200/60">
            Basics
          </h4>
          <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label class="flex flex-col gap-2 text-sm">
              ...
              <input
                ...
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
              />
            </label>
            <label class="flex flex-col gap-2 text-sm">
              Age
        ...
        <div>
          ...
          <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label class="flex flex-col gap-2 text-sm">
              Diet Preference
              <select
                bind:value={form.dietPreference}
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
              >
            ...
            <label class="flex flex-col gap-2 text-sm">
              ...
              <input
                bind:value={form.dailyWaterIntake}
                type="number"
                min="0"
                step="0.1"
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
        ...
        <div>
          ...
          <div class="mt-4 grid gap-4 md:grid-cols-2">
            <div class="md:col-span-2">
              ...
              <div class="mt-3 flex flex-wrap gap-3">
                {#each medicalConditionOptions as condition}
                  <label
                    class="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm"
                  >
                    <input
                      type="checkbox"
            ...
            <label class="flex flex-col gap-2 text-sm md:col-span-2">
              ...
              <textarea
                bind:value={form.injuriesOrLimitations}
                rows="3"
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
              ></textarea>
            </label>
        ...
        <div>
          ...
          <div class="mt-4 grid gap-4 md:grid-cols-2">
            <label class="flex flex-col gap-2 text-sm">
              ...
              <input
                bind:value={form.workoutHoursPerDay}
                type="number"
                min="0.5"
                max="3"
                step="0.5"
            ...
            <label class="flex flex-col gap-2 text-sm">
              ...
              <input
                bind:value={form.workoutDaysPerWeek}
                type="number"
                min="1"
                max="7"
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
            ...
            <label class="flex flex-col gap-2 text-sm md:col-span-2">
              ...
              <select
                bind:value={form.preferredWorkoutTime}
                class="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 outline-none"
              >
      ...
      <div class="mt-6 flex justify-end gap-3">
        <button
          class="rounded-full border border-white/10 px-5 py-3"
